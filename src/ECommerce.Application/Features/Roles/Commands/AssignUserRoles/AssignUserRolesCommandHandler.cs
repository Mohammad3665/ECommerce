...
public class AssignUserRolesCommandHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUser) : IRequestHandler<AssignUserRolesCommand, Result>
{
    public async Task<Result> Handle(AssignUserRolesCommand request, CancellationToken cancellationToken)
    {
        ...
        if (hasSuperAdminInRequest && currentUserMaxLevel < 100)
            return new Error("Role.UnauthorizedAssignment", "شما سطح دسترسی لازم برای اعطای نقش مدیریت کل (Super Admin) را ندارید.", ErrorType.Validation);
        ...
        if (roles.Count() != cleanedSlugs.Count())
            return new Error("Role.SomeRolesNotFound", "یک یا چند نقش ارسال شده در سیستم یافت نشد.", ErrorType.NotFound);

        var targetMaxRoleLevel = roles.Max(r => r.Level);
        if (targetMaxRoleLevel >= currentUserMaxLevel && currentUserMaxLevel < 100)
            return new Error("Role.InvalidLevelAssignment", "شما نمی‌توانید نقشی هم‌سطح یا بالاتر از خودتان را به کاربری تخصیص دهید.", ErrorType.Validation);
        ...
        return saveResult.IsFailure ?
            new Error("Role.AssignmentFailed", "خطایی در تخصیص نقش‌ها رخ داد.", ErrorType.Unexpected) :
            Result.Success();
    }
}
